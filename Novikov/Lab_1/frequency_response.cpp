...
int main() {
    ...
    gnuplot_amplitude << "unset logscale x" << endl;  // УБИРАЕМ логарифмическую шкалу
    gnuplot_amplitude << "set xrange [0:300]" << endl;  // РАВНОМЕРНАЯ шкала до 400
    ...
    gnuplot_amplitude << "set arrow from first 0, graph 0 to first 0, graph 1 nohead lc 'black' lw 1.5" << endl;
    gnuplot_amplitude << "plot 'amplitude_data.txt' with lines lw 3 lc rgb 'red' title 'АЧХ'" << endl;
    
        // 3. ЛАЧХ (РАВНОМЕРНАЯ ОСЬ X, ТОЛЬКО ТОЧНАЯ)
    gnuplot_lach << "set terminal pngcairo size 800,600 enhanced font 'Arial,14'" << endl;
    gnuplot_lach << "set output 'lach_plot.png'" << endl;
    gnuplot_lach << "set title 'Логарифмическая амплитудно-частотная характеристика (ЛАЧХ)'" << endl;
    gnuplot_lach << "set xlabel 'ω, рад/с' font 'Arial,12'" << endl;
    gnuplot_lach << "set ylabel 'L(ω), дБ' font 'Arial,12'" << endl;
    gnuplot_lach << "unset logscale x" << endl;  // УБИРАЕМ логарифмическую шкалу
    gnuplot_lach << "set xrange [0:400]" << endl;  // РАВНОМЕРНАЯ шкала до 400
    ...
    gnuplot_lach << "set arrow from first 0, graph 0 to first 0, graph 1 nohead lc 'black' lw 1.5" << endl;
    gnuplot_lach << "plot 'bode_data.txt' using 1:2 with lines lw 3 lc rgb 'green' title 'ЛАЧХ'" << endl;
    
    // 4. ФЧХ (РАВНОМЕРНАЯ ОСЬ X, ТОЛЬКО ТОЧНАЯ)
    ...
    gnuplot_phase << "unset logscale x" << endl;  // УБИРАЕМ логарифмическую шкалу
    gnuplot_phase << "set xrange [0:400]" << endl;  // РАВНОМЕРНАЯ шкала до 400
    ...
    gnuplot_phase << "set arrow from first 0, graph 0 to first 0, graph 1 nohead lc 'black' lw 1.5" << endl;
    gnuplot_phase << "plot 'phase_data.txt' with lines lw 3 lc rgb 'purple' title 'ФЧХ'" << endl;
    
    // 5. ЛФЧХ (РАВНОМЕРНАЯ ОСЬ X, ТОЛЬКО ТОЧНАЯ)
    gnuplot_lfch << "set terminal pngcairo size 800,600 enhanced font 'Arial,14'" << endl;
    gnuplot_lfch << "set output 'lfch_plot.png'" << endl;
    gnuplot_lfch << "set title 'Логарифмическая фазо-частотная характеристика (ЛФЧХ)'" << endl;
    gnuplot_lfch << "set xlabel 'ω, рад/с' font 'Arial,12'" << endl;
    gnuplot_lfch << "set ylabel 'φ(ω), градусы' font 'Arial,12'" << endl;
    gnuplot_lfch << "unset logscale x" << endl;  // УБИРАЕМ логарифмическую шкалу
    gnuplot_lfch << "set xrange [0:400]" << endl;  // РАВНОМЕРНАЯ шкала до 400
    ...
    gnuplot_lfch << "set arrow from first 0, graph 0 to first 0, graph 1 nohead lc 'black' lw 1.5" << endl;
    gnuplot_lfch << "plot 'lfch_data.txt' using 1:2 with lines lw 3 lc rgb 'purple' title 'ЛФЧХ'" << endl;
    
        // 6. Действительная и мнимая части (РАВНОМЕРНАЯ ОСЬ X до 400, АВТОМАТИЧЕСКИЙ Y)
    ...
    gnuplot_real_imag << "set title 'Действительная и мнимая части передаточной функции' font 'Arial,14'" << endl;
    gnuplot_real_imag << "set xlabel 'ω, рад/с' font 'Arial,12'" << endl;
    gnuplot_real_imag << "set ylabel 'Re(H), Im(H)' font 'Arial,12'" << endl;
    gnuplot_real_imag << "unset logscale x" << endl;  // УБИРАЕМ логарифмическую шкалу
    gnuplot_real_imag << "set xrange [0:200]" << endl;  // РАВНОМЕРНАЯ шкала от 0 до 400
    // УБИРАЕМ set yrange - оставляем автоматическое масштабирование по Y
    ...
    gnuplot_real_imag << "set arrow from first 0, graph 0 to first 0, graph 1 nohead lc 'black' lw 1.5" << endl;
